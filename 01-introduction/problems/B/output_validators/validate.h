 ...
 * problem format. Edited by Alon Krymgand, Originally from:
 * https://tinyurl.com/y2nssela
 *
...
class Judge {
   ...
   public:
    ...
    std::istream author_out;

    Judge(int argc, char **argv) : author_out(std::cin.rdbuf()) {
        if (argc < 4) {
            fprintf(stderr,
                    "%s: judge_in judge_ans feedback_dir < author_out\n",
                    argv[0]);
            judge_error(
                "Usage: %s judge_in judge_ans feedback_dir < author_out\n",
                argv[0]);
        }
   ...
   protected:
    ...
    void vreport_feedback(const std::string &category, const std::string &msg,
                          va_list pvar) {
        std::ostringstream fname;
        if (feedbackdir) fname << feedbackdir << '/';
        fname << category;
        FILE *f = fopen(fname.str().c_str(), "a");
        assert(f);
        if (testcase) fprintf(f, "Test case %d:\n", testcase);
